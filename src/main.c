...
#define PROGRESS_PORT 35789

static void Exit(int ec) {
    if (tcp_cli_hnd) {
        DestroyClient(tcp_cli_hnd);
    }
    exit(ec);
}
...
int main(int argc, char **argv)
{
    ...
    tcp_cli_hnd = CreateClient(PROGRESS_PORT);
    if (!tcp_cli_hnd) {
        fprintf(stderr, "Error: Failed to create tcp client\n");
        Exit(EX_IOERR);
    }
    // NetPerr(tcp_cli_hnd, "SendData!");
...
status_again:
	...
	case MODE_UPLOAD:
        ...
        memset(transfer_file_name, 0x0, MAX_FILENAME_LEN);
        file_name_len = strlen(file.name);
        if (file_name_len > MAX_FILENAME_LEN) {
            file_name_len = MAX_FILENAME_LEN;
        }
        memcpy(transfer_file_name, file.name, file_name_len);
