...
// TBD:增加进度更新支持
// TBD:增加错误码及错误处理
int dfu_progress_bar(const char *desc, unsigned long long curr,
		unsigned long long max)
{
    static int progress_dbg = 0;
    ...
    if (tcp_cli_hnd) {
        ...
        if (progress_dbg != prog) {
            progress_dbg = prog;
            printf("\n !!!Progress:%d \n", prog);
        }
        if (SendProgress(tcp_cli_hnd, transfer_file_name, prog)) {
            fprintf(stderr, "Tcp send progress notification failed!\n");
            return -1;
        }
    } else {
        fprintf(stderr, "Tcp is not connected, progress can not be sent!\n");
        return -1;
    }
#if 0
...
void dfu_load_file(struct dfu_file *file, enum suffix_req check_suffix, enum prefix_req check_prefix)
{
	...
	if (!strcmp(file->name, "-")) {
		...
		while (read_total < file->size.total) {
            dfu_off_t to_read = file->size.total - read_total;
